...
# PINNED ENGINE. plc2 must be labelled by the SAME binary as plc1 or the merged
# corpus is inhomogeneous, so we install from a PRIVATE copy under the run
# prefix and verify its sha256. We deliberately do NOT read $S3/evallab/wheel/
# (a concurrent job may publish a newer wheel there) and there is NO build
# fallback: a wrong engine is worse than a dead box.
python3.11 -m venv /opt/venv
/opt/venv/bin/pip3 install -q --upgrade pip
mkdir -p /opt/wheel
WHL=/opt/wheel/poke_engine-0.0.59-cp311-cp311-linux_x86_64.whl
$AWS s3 cp $S3/evallab/__POSRUN__/wheel/poke_engine-0.0.59-cp311-cp311-linux_x86_64.whl "$WHL" --only-show-errors

#!/bin/bash
# evallab FAST-PATH training box. One job, then self-terminate.
#
# Structure copied verbatim from userdata_canary.sh (the proven one). Only the
# stage list differs: no pretrain cache, no pre1 sample, no verify -- fetch the
# whole plc1 cache, train N seeds FROM RANDOM INIT concurrently, pick, publish.
#
...
status "STAGE train seeds=__SEEDS__ steps=__STEPS__ threads=__THREADS__"
PIDS=""
for S in $(echo __SEEDS__ | tr ',' ' '); do
  OMP_NUM_THREADS=__THREADS__ nohup /opt/venv/bin/python vt_fast.py train /opt/work \
...
status "STAGE pick"
/opt/venv/bin/python vt_fast.py pick /opt/work --seeds __SEEDS__ > /opt/work/pick.log 2>&1
push
status "pick done"

$AWS s3 cp /opt/work/ $PUB/$TAG/ --recursive --exclude "*" \
    --include "REPORT.*.json" --include "ckpt_fast_s*.pt" \

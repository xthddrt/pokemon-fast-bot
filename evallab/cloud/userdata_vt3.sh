...
# ---- FAIL-LOUD CANARY: the ARM A CONTROL and one ADDITIVE cell must both
# ---- produce a sane Brier before the grid starts. The control also re-asserts
# ---- arm A's published parameter count (1,091,541 at 256x512) -- a wrong stub,
# ---- a broken cache or a mis-shipped add-on cache dies HERE, not in the grid.
cd /opt/evallab
/opt/venv/bin/python vt_train.py --arm old --cap 256,512 --seed 0 --epochs 2 \
    --out /opt/out/canary_ctl.json > /opt/out/canary_ctl.log 2>&1
/opt/venv/bin/python vt_train.py --arm old --add setup,caps,ko,speed,tera \
    --cap 256,512 --seed 0 --epochs 2 \
    --out /opt/out/canary_add.json > /opt/out/canary_add.log 2>&1
/opt/venv/bin/python - <<'CANARY'
import json
exp = {"ctl": 1091541, "add": 1342165}
for a in ("ctl", "add"):
    r = json.load(open("/opt/out/canary_%s.json" % a))
    b = r["test"]["all"]
    assert 0.005 < b < 0.09, "canary %s Brier %.5f out of range" % (a, b)
    assert r["params"] == exp[a], "canary %s params %d != %d" % (a, r["params"], exp[a])

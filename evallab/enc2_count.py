...
def encode_columnar(C, vocab, S=None, L=DEFAULT_LAYOUT, debug=None):
    ...
    for lv in SETUP_LEVELS:
        ...
        d2 = combine(dmg_moves, numerator(_sides_of(st)[0]))
    ...
    # tera-enabled sweep: tera alone (it turns a non-kill into a kill on the
    # active) composed with either an existing +1 sweep or a setup move
    tera_sweep = (untera & (off_gain >= 1.0)
                  & (sweep1 | (S.setup_present.reshape(n, 2, 6) > 0)))

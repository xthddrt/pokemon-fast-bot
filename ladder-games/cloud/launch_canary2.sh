#!/bin/bash
# Launch the relay-architecture canary box (bots on AWS, websocket via the
# Mac relay tunnel). Reuses the canary1 code tarball. Prints the instance id.
set -euo pipefail

ROOT="/Users/sallyliu/pokemon-fast-bot"
AWS="/Users/sallyliu/.awscli-venv/bin/aws"
BUCKET="pokebot-valuenet-389825051723"
PREFIX="${CANARY_PREFIX:-ladder-fleet/canary3}"
REGION="us-east-2"
SCRATCH="$(mktemp -d)"
...
$AWS s3 cp "$SCRATCH/code.tar.gz" "s3://$BUCKET/$PREFIX/code.tar.gz" --quiet
...
PS_PASSWORD=$(grep '^PS_PASSWORD=' "$ROOT/.env" | cut -d= -f2- | tr -d '"')
{
  ...
  cat "$ROOT/ladder-games/cloud/canary2_bootstrap.sh"
...
$AWS ec2 run-instances \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=ladder-canary2}]' \
